package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-podium/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-podium"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/feedback",s.handleList)
    s.mux.HandleFunc("POST /api/feedback",s.handleCreate)
    s.mux.HandleFunc("PUT /api/feedback/{id}",s.handleUpdate)
    s.mux.HandleFunc("DELETE /api/feedback/{id}",s.handleDelete)
